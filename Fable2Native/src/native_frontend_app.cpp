...
private:
    ...
    void render_native_main_menu(ID3D12GraphicsCommandList* command_list) {
        ...
        const auto add_native_text = [&](std::string_view text, float x, float y,
                                         float size, std::uint32_t color) {
            emit_text(scene, text, x, y, size, color);
        };
        ...
        const auto add_body_three_slice = [&](float x0, float y0, float x1, float y1,
            ...
            // (v0.203..0.279 = y104..143, measured alpha 255) and the leather GRAIN from menu_surface
            // as the detail. combine_detail => out.rgb = color.rgb * menu_surface.rgb (leather),
            // out.a = color.a * ability_elements.a (= 1 at the opaque core) => fully opaque leather.
            // (The translucency was never this material — it was an ungated ImGui dark veil drawn on
            // top; that is now gated to the ImGui fallback only.)
            const auto add_body_slice = [&](float sx0, float sx1, float su0, float su1) {
                const auto before = scene.size();
                add(f2::NativeUiAsset::AbilityElements, sx0, y0, sx1, y1,
                    su0, 104.0f / 512.0f, su1, 143.0f / 512.0f, color);
        ...
        // includes Options, its window bg is cleared, and the ImGui Options blocks are skipped).
        if (game_.frontend.state() == f2::FrontendState::Options) {
            render_native_options_chrome(scene);
            if (game_.frontend.options_page_open()) render_native_options_page(scene);
        }
    ...
    void render_native_video(ID3D12GraphicsCommandList* command_list) {
        ...
        f2::render::UiDrawList scene;
        scene.add_sprite(texture_id_for(kVideoTextureKey, video_gpu_handle_), 0.0f, 0.0f,
                         static_cast<float>(width_), static_cast<float>(height_), 0.0f, 0.0f, 1.0f,
                         1.0f, 0xffffffffu);
        native_ui_renderer_.render(command_list, width_, height_, scene.quads(),
                                   [this](f2::render::TextureId id) { return resolve_ui_texture(id); });
    }
